...
const timelineHeight = $$('.timeline__data').length * ($$('.timeline__data')[0].offsetHeight + 10)
...
let scrollHandler = () => { // Menu active when user scroll
    window.addEventListener('scroll', () => {
        ...
        // ======================= Education =======================
        if (verticalPosition >= educationPosY && verticalPosition < experiencePosY) {
            $('.subnav.active').classList.remove('active')
...
let effect = { // Change Background and Data Home Section
    home: {
        ischange: false,
        background:
        {
        ...
        data: {
            aboutMe:
            {
                ...
                button: 'Download CV <i class="fas fa-cloud-download-alt home__desc__button__icon"></i>'
            },
            developer:
            {
                title: 'I am <br> Developer',
                desc: '100% html5 bootstrap templates Made by <a href="#" class="home__desc__link">colorlib.com</a>',
                button: 'View Porfolio <i class="fas fa-suitcase home__desc__button__icon"></i>'
            }
    ...
    homeEffectHandler: function () {
        ...
        gsap.from(myE, { // Start point animation    
            y: 100,
            opacity: 0
        })
        gsap.to(myE, { // End point animation
            y: 0,
            opacity: 1,
            duration: 1.2
        })
...
const about = {
    ...
    effectHandler: function () {
        this.selectors.forEach((selector, index) => {
            ...
            let setTimeDelay = (index) => { // Set time delay for each selector
                let timeDelay
                switch (index) {
...
const tablet = {
    ...
    inforTabHanlder: function () { // Show information tab when user click
        toggleBtnInfo.onclick = () => {
            informationTab.classList.toggle('active',
    ...
    eventHandler: function () {
        window.addEventListener('scroll', () => { // Hide information tab when user scroll
        ...
        informationModal.addEventListener('click', () => { // Hide information tab when user click modal
            if (informationTab.classList.contains('active')) {
                informationTab.classList.remove('active')
                informationModal.classList.remove('active')
            }
